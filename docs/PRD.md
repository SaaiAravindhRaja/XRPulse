...
## 1. Product Vision
**XRPulse** is a decentralized marketplace where rural clinics can list high-cost medical equipment (MRIs, CT Scanners) for funding. Investors fund these using **RLUSD**, receiving fractional ownership tokens (RWAs) that yield returns from the clinic's lease payments.
...
*   **XRPL Tech:** 
    ...
    2.  **Issue Currency:** Create `PULSE-MRI-001` tokens (Fractional shares).
...
*   **XRPL Tech:** 
    1.  **Trustline:** Investor wallet sets trustline for `PULSE-MRI-001`.
    2.  **Payment:** Investor sends `RLUSD` (Testnet token) to Clinic's Escrow.
    3.  **Swap:** Clinic sends `PULSE-MRI-001` to Investor.

### Slice D: Yield Distribution (Impact)
*   **User Flow:** "Simulate Month End" button on Admin Dashboard.
*   **XRPL Tech:** Clinic wallet sends `RLUSD` payment to all holders of `PULSE-MRI-001` based on balance.
